            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(feedData))
                {
                    ...
                    response = feed.Items.Select(x => new RssFeedModel
                    {
                        ...
                        ImageLink = GetUrlLinkFromHtml(((TextSyndicationContent)(x.Content ?? x.Summary)).Text.AsString()),
                        ImageUrl = GetImageLinkFromHtml(((TextSyndicationContent)(x.Content ?? x.Summary)).Text.AsString()),
