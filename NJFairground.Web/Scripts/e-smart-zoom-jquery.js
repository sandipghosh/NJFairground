        ...
        function animate(target, globalLeft, globalTop, width, height, duration, callback) {
            ...
            if (smartData.transitionObject != null) { // use css transition if supported
                ...
                if (smartData.transitionObject.css3dSupported) // use css 3d translate if supported
                    cssObject[smartData.transitionObject.transform] = 'translate3d(' + left + 'px, ' + (top > 0 ? 0 : top) + 'px, 0) scale3d(' + width / originalSize.width + ',' + height / originalSize.height + ', 1)';
                else
                    cssObject[smartData.transitionObject.transform] = 'translateX(' + left + 'px) translateY(' + (top > 0 ? 0 : top) + 'px) scale(' + width / originalSize.width + ',' + height / originalSize.height + ')';
